        ...
        if(!empty($_FILES)){
            $x = move_uploaded_file($_FILES['file']['tmp_name'],'uploads/files/'.$_FILES['file']['name']);
             ...
             $post->media_url = 'uploads/files/'.$_FILES['file']['name'];
                        ...
                        //  $stamp = imagecreatefrompng('watermark.jpg');
                        //  $im = imagecreatefromjpeg('uploads/files/'.$_FILES['file']['name']);
             // Load the stamp and the photo to apply the watermark to
            ...
            // $marge_right = 10;
            // $marge_bottom = 10;
            // $sx = imagesx($stamp);      //return stamp width
            // $sy = imagesy($stamp);      //return stamp height
            // Copy the stamp image onto our photo using the margin offsets and the photo
             // width to calculate positioning of the stamp.
            //  imagecopy($im, $stamp, imagesx($im) - $sx - $marge_right, imagesy($im) - $sy - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp));
            //
            // $post->media_url= imagepng($im);
